...
def calc_mean(x): #Calculate the mean of all 3 or 2 scores [elasticsearch score,movie average rating,user rating]
    counter = sums = 0
    for i in x:
        if(not np.isnan(i)):
            counter += 1
...
movies_mo = ratings.groupby('movieId', as_index=False)['rating'].mean() #Average rating for every movie
...
while True: #Query iteration
    ...
    es_results = es.search(index="movies", body={"size":100,"query": {"match": {'title': query}}}) #Elasticsearch score

    #Retrieve only usefull information from elasticsearch and rescale values
    ...
    df_es['es_score'] = df_es['es_score'].apply(lambda x:x*scalar) #Setting maximum value to 5 (rescaling every value)

    #Calculate the new score values and print it
    ...
    metric_results['new_score'] = metric_results['new_score'].apply(calc_mean) #Calculate the mean
